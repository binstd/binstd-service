...
router.post('/signin', async (req, next) => {
  ...
  console.log(req.request.body);
	...
	if (req.request.body.telephone && req.request.body.password) {
		...
		keyinfo = await db.query('SELECT uid,telephone,address,privatekey,username,img_url FROM user WHERE telephone=? AND password=?', [telephone, password]);
	}
	console.log(keyinfo);
	if (keyinfo.length == 1) {
